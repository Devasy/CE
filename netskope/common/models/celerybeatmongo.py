"""Models for the celery beat mongo database."""
import datetime
...
def get_periodic_task_collection():
    """Get the periodic task collection name."""
...
class PeriodicTask(DynamicDocument):
    ...
    class Interval(EmbeddedDocument):
        """Mongo database model that represents a periodic task interval."""
        ...
        @property
        def schedule(self):
            """Get the schedule from the interval."""
        ...
        def __unicode__(self):
            """Get the unicode representation of the interval."""
            if self.every == 1:
                return 'every {0.period_singular}'.format(self)
            return 'every {0.every} {0.period}'.format(self)

    class Crontab(EmbeddedDocument):
        """Mongo database model that represents a periodic task crontab."""
        ...
        @property
        def schedule(self):
            """Get the schedule from the crontab."""
        ...
        def __unicode__(self):
            """Get the unicode representation of the crontab."""
            def rfield(f): return f and str(f).replace(' ', '') or '*'
    ...
    @property
    def schedule(self):
        """Get the schedule from the periodic task."""
        if self.interval:
    ...
    def __unicode__(self):
        """Get the unicode representation of the periodic task."""
